...
def render_site_methodology(ds: xr.Dataset, out_path: Path) -> None:
    ...
    ax1.set_title("Site: channel polyline on the treatment AOI\n"
                  "Background: mean NDVI 2018–2025 post-construction",
                  fontsize=11)
    ax1.set_xlabel("UTM 37S easting (m)")
    ax1.set_ylabel("UTM 37S northing (m)")
    ...
    ax2.set_title("Methodology: distance bands around the channel",
                  fontsize=11)
    ax2.set_xlabel("UTM 37S easting (m)")
    ax2.set_ylabel("UTM 37S northing (m)")
    handles = [
    ...
    fig.suptitle(
        ...
        f"(Machakos, Kenya). Treatment AOI: 1 km × 1 km. "
...
def render_yearly_ndvi_grid(
    ...
    for ax, year in zip(axes_flat, years):
        ...
        period = "pre-construction" if year < 2019 else "post-construction"
        drought_flag = " (drought)" if 2020 <= year <= 2023 else ""
        ax.set_title(f"{year} – {period}{drought_flag}", fontsize=9)
        ax.set_xticks([]); ax.set_yticks([])
    for ax in axes_flat[len(years):]:
        ax.axis("off")
    fig.suptitle(
        f"Annual median NDVI in the treatment AOI, 2016–2025 ({season_label})",
        fontsize=12, y=1.0,
    )
    fig.tight_layout()
...
def render_timeseries_with_precip(
    ...
    since the control AOI was not selected by a defensible procedure.
    """
    if aois is None:
    ...
    ax1.axvspan(config.DROUGHT_START, config.DROUGHT_END, color="red", alpha=0.08,
                label="2020–2023 Horn of Africa drought")
    ax1.set_ylabel(f"Vegetation fraction (NDVI > {config.NDVI_VEG_THRESHOLD})")
    ax1.set_title("Vegetation cover and rainfall, 2016–2025", fontsize=12)
...
def render_per_pixel_change_simplified(
    ...
    Differences from render_per_pixel_change:
        - Light gaussian smoothing to reduce single-pixel noise.
        - Discrete five-band colour scheme (Large loss / Some loss / Little
          change / Some gain / Large gain) instead of a continuous gradient.
        - Channel polylines overlaid in cyan.
        - Dam location marked with a star.
        - Scale bar showing 200 m.
        - North arrow.
        - Legend in plain language (no technical units).
    """
    ...
    # Light spatial smoothing (sigma ~= 2 pixels = 20 m). Keep NaNs as NaN.
    arr_smooth = arr.copy()
    ...
    # Discrete bands (in change-of-fraction units, where 0.10 = 10 percentage points)
    ...
    # Scale bar (200 m horizontal at lower-right)
    bar_len_m = 200
    bar_x0 = xmax - 280
    bar_y0 = ymin + 60
    ...
    ax.set_title(
        ...
        f"(post-construction 2019–2025 vs pre-construction 2016–2018)",
        fontsize=11,
    )
...
def render_corridor_decay_comparison(
    ...
    Used to compare the upstream and downstream channel segments side-by-side.
    """
